...
urlpatterns = [
    ...
    # --- ЖАҢА: NOTES (Заметкалар) ---
    path('notes/', views.note_manager, name='note-manager'),

    # --- ЖАҢА: STORIES (Историялар) ---
    path('stories/', views.story_manager, name='story-list'), # GET (тізім) және POST (жүктеу)
    path('stories/<int:pk>/', views.story_manager, name='story-detail'), # DELETE (өшіру)
]
...
def post_manager(request, pk=None):
    if request.method == 'GET':
        ...
        if post.author != request.user:
            return Response({"error": "Бұл сіздің постыңыз емес"}, status=403)
...
# 3. COMMENTS & LIKES (Пікірлер мен Лайктар)
@api_view(['GET', 'POST', 'PUT', 'DELETE'])
@permission_classes([IsAuthenticated])
def post_interaction(request, post_id, comment_id=None):
    ...
    if 'comment' in request.path:
        # GET: Посттың барлық пікірлерін көру
        if request.method == 'GET':
            comments = post.comments.all() # Модельде related_name='comments' болуы керек
            serializer = CommentSerializer(comments, many=True)
            return Response(serializer.data)

        # POST: Жаңа пікір жазу
        elif request.method == 'POST':
        ...
        # PUT: Пікірді өзгерту
            ...
            if comment.author != request.user:
                return Response({"error": "Бұл сіздің пікіріңіз емес"}, status=403)
        ...
        # DELETE: Пікірді өшіру
        elif request.method == 'DELETE':
            comment = get_object_or_404(Comment, pk=comment_id, post=post)
            # Тек пікір авторы немесе пост иесі өшіре алады
            if comment.author == request.user or post.author == request.user:
                comment.delete()
                return Response({"message": "Пікір өшірілді"}, status=204)
            return Response({"error": "Өшіруге рұқсат жоқ"}, status=403)
...
def user_follow(request, pk):
    ...
    if request.method == 'GET':
        ...
        if request.user == user:
            return Response({"error": "Өзіңізге жазыла алмайсыз"}, status=400)
        # Бұл жерде get_or_create қолдану керек
...
def add_media(request, post_id):
    ...
    if post.author != request.user:
        return Response({"error": "Бұл сіздің постыңыз емес"}, status=status.HTTP_403_FORBIDDEN)
